...
MODEL_PATH = '../net/n/2_gru/imm_param_net.pth'
SCALER_PATH = '../net/n/2_gru/scaler_params.json'
WINDOW_SIZE = 90
...
class ParamPredictorLSTM(nn.Module):
    # 虽然名字还叫 LSTM (为了省去改名的麻烦)，但内部已经换成了 GRU
    def __init__(self, input_dim=9, hidden_dim=64):  # [建议1] hidden_dim 改为 64
        ...
        self.fc = nn.Sequential(
            nn.Linear(hidden_dim, 32),  # 中间层也可以适当减小
            nn.ReLU(),
            nn.Dropout(0.4),  # [建议2] 增加 Dropout 到 0.4
            nn.Linear(32, 9)
        )
